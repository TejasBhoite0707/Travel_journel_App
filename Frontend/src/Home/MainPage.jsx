...
<motion.p className='text-xl mt-20 text-center text-[#FDF6E3]'
initial={{ opacity: 0 }}
animate={{ opacity: 1 }}
...
>
  <Link to='/signup'><button className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-6 py-3 rounded-lg hover:scale-105 transition">
  Get Started
</button></Link>  
...
<motion.div className='flex flex-row space-x-7 items-center text-white mt-16'
initial='hidden'
animate='visible'
variants={{
    visible:{
...
>
    {
        [ {
          ...
          },
          {
            ...
            text: 'Safe And Private Diary for Your Journey',
          },].map((item,i)=>(
            <motion.div
            key={i}
            className='flex flex-row items-center space-x-3 hover:scale-105 transition'
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
